 ...
 * @author baranowb
 */
public class Mtp2Impl implements Mtp2 {
	...
	private ArrayList<Mtp2Listener> listeners = new ArrayList<Mtp2Listener>();
	...
	public void _startLink() throws IOException {
		...
		// t.start();
		state = Mtp2Impl.MTP2_OUT_OF_SERVICE;
		//start_T17();
	...
	private void processFrame() {
		...
		if (this.enableSuTrace && enabledL2Debug) {
			...
			sbb.append("Received frame, type = ").append(type).append(
					" sequencing, (fsn = ").append(fsn).append(", fib = ")
					.append(fib).append(", bsn = ").append(bsn).append(
		...
		if ((this.retransmissionFSN_LastAcked <= this.retransmissionFSN_LastSent && (bsn < this.retransmissionFSN_LastAcked || bsn > this.retransmissionFSN_LastSent))
			...
			if (this.bsnErrors > 2) {
				...
				alignmentBroken("Broken BSN constrains: fsn_lasAcked = "
						+ this.retransmissionFSN_LastAcked
						+ ", fsn_LastSent = " + this.retransmissionFSN_LastSent
						+ ", bsn = " + bsn);
			}
		...
		while (i < len) {
			...
			default:
				if (rxLen > 279) {
					...
					eCount = 0;
					countFrame();
					countError("Overlong MTP frame, entering octet mode on link '"
							+ name + "'");
	...
	private void alignmentNotPossible(String cause) {
		...
		if (enabledL2Debug) {
			trace("Alignment not possible, initiating T17 for restart. Cause: "
					+ cause);
		}
	}
	...
	private void alignmentBroken(String cause) {
		...
		if (enabledL2Debug) {
			trace("Alignment broken, initiating T17 for restart. Cause: "
					+ cause);
		}
	}
			...
			if (this.aermEnabled)
				if (eCount >= aermThreshold) {
					...
					if (this.provingAttempts < 5) {
						...
						if (enabledL2Debug) {
							trace("Exceeded AERM threshold[ " + aermThreshold
									+ " ] errors[ " + eCount
									+ " ], proving attempts[ "
									+ provingAttempts + " ]");
						}
	...
	public void start_T1_SLTM() {
		...
		this.T1_SLTM = mtpTimer.schedule(this.t1Action_SLTM,
				Mtp3._TIMEOUT_T1_SLTM, TimeUnit.SECONDS);
	}

	public void start_T2_SLTM() {
		stop_T2_SLTM();
		this.T2_SLTM = mtpTimer.schedule(this.t2Action_SLTM,
				Mtp3._TIMEOUT_T2_SLTM, TimeUnit.SECONDS);
	}
	...
	public void trace(String msg) {
		...
		sb.append("\n").append(System.currentTimeMillis()).append(" Link [")
				.append(name).append("] [").append(" state = ").append(
						_STATE_NAMES[state]).append(" ]").append(" fsn = ")
				.append(retransmissionFSN_LastSent).append(", fsn_acked = ")
				.append(this.retransmissionFSN_LastAcked).append(", rtrFSN = ")
				.append(this.retransmissionFSN).append(", fib = ").append(
						this.sendFIB).append(", bsn = ").append(this.sendBSN)
				.append(", bib = ").append(this.sendBIB).append(") ").append(
						msg);
