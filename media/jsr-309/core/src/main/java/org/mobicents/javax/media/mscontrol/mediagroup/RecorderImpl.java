	...
	private class StopTx implements Runnable, JainMgcpExtendedListener {
		...
		public void run() {
			try {
				...
				mgcpWrapper.addListener(reqId, this);
				...
				logger.error(e);
				RecorderEventImpl event = new RecorderEventImpl(this.recorder, RecorderEvent.RECORD_COMPLETED, false,
		...
		private void processReqNotificationResponse(NotificationRequestResponse responseEvent) {
			...
			default:
				...
				event = new RecorderEventImpl(this.recorder, RecorderEvent.RECORD_COMPLETED, false, MediaErr.UNKNOWN_ERROR,
	...
	private class StartTx implements Runnable, JainMgcpExtendedListener {
		...
		public void run() {
			...
			try {
				...
				for (DefaultEventGeneratorFactory genfact : medGrpConfig.getRecorderConfig().getEventGenFctList()) {
		...
		public void processMgcpCommandEvent(JainMgcpCommandEvent command) {
			...
			for (EventName observedEvent : observedEvents) {
				...
				case AUMgcpEvent.OPERATION_FAIL:
					mgcpWrapper.removeListener(notify.getRequestIdentifier());
					event = new RecorderEventImpl(this.recorder, RecorderEvent.RECORD_COMPLETED, false,
