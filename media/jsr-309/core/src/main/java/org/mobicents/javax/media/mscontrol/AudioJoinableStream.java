...
public class AudioJoinableStream implements JoinableStream {
	...
	private Runnable getJoinTx(Direction direction, Joinable other, Serializable context) throws MsControlException {
		Runnable tx = null;
		...
		if (thisDir != null) {
			...
			if (!thisDir.equals(direction)) {
				...
				tx = new ModifyTx(this, thisConnId, direction, audioJoiStreamOther, otherConnId, context);
			} else {
			...
			tx = new JoinTx(this, direction, audioJoiStreamOther, context);
		}
		return tx;
	}

	private Runnable getUnjoinTx(Joinable other, Serializable context) throws MsControlException {
		Runnable tx = null;
		if (!this.container.state.equals(MediaObjectState.RELEASED)) {
			...
			if (!audioJoiStreamOther.container.state.equals(MediaObjectState.RELEASED)) {
				...
				tx = new UnJoinTx(this, thisConnId, audioJoiStreamOther, otherConnId, context);
			}
		}
		return tx;
	}
	...
	public void join(Direction direction, Joinable other) throws MsControlException {
		...
		try {
			Runnable tx = getJoinTx(direction, other, null);

			if (tx != null) {
				this.semaphore = new Semaphore(0);
				Provider.submit(tx);
				// TODO : Session Timeout and DLCX for previous CRCX command???

				System.out.println("semaphore.tryAcquire() = " + this.semaphore);
				try {
					boolean acquired = this.semaphore.tryAcquire(5000, TimeUnit.MILLISECONDS);
					if (!acquired) {
				...
				if (this.anEvent.getError() != MediaErr.NO_ERROR) {
					throw new MsControlException("Join failed Error = " + this.anEvent.getError() + " "
							+ this.anEvent.getErrorText());
				}
			}
		} finally {
			// Clean the Semaphore
	...
	public void joinInitiate(Direction direction, Joinable other, Serializable context) throws MsControlException {
		Runnable tx = getJoinTx(direction, other, context);
		if (tx != null) {
			Provider.submit(tx);
		}
	}
	...
	public void unjoin(Joinable other) throws MsControlException {
		...
		try {
			Runnable tx = getUnjoinTx(other, null);
			...
			if (tx != null) {
				...
				if (this.anEvent.getError() != MediaErr.NO_ERROR) {
					throw new MsControlException("UnJoin failed Error = " + this.anEvent.getError() + " "
							+ this.anEvent.getErrorText());
				}
			}
		} finally {
	...
	public void unjoinInitiate(Joinable other, Serializable context) throws MsControlException {
		Runnable tx = getUnjoinTx(other, context);
		if (tx != null) {
			Provider.submit(tx);
		}
	}

	private void updateUnjoined(JoinEvent anEvent, ConnectionIdentifier thisConnId, ConnectionIdentifier otherConnId,
		...
		//TODO : Fire event even for sync unjoin call?
		this.container.updateUnjoined(anEvent, thisConnId, otherConnId, otheContainer, error);

	}

	private void updateJoined(JoinEvent anEvent, ConnectionIdentifier thisConnId, ConnectionIdentifier otherConnId,
			AbstractJoinableContainer otheContainer, boolean error) {
		...
		//TODO : Fire event even for sync join call?
		this.container.updateJoined(anEvent, thisConnId, otherConnId, otheContainer, error);
	}
	...
	private class UnJoinTx implements Runnable, JainMgcpExtendedListener {
		...
		UnJoinTx(AudioJoinableStream thisAudJoiStr, ConnectionIdentifier thisConnId,
				AudioJoinableStream otherAudJoiStr, ConnectionIdentifier otherConnId, Serializable context) {
		...
		public void run() {
			...
			try {
				mgcpWrapper.addListener(this.thisTx, this);
				...
				JoinEvent joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
						this.otherAudJoiStr.container, container, JoinEvent.UNJOINED, false, MediaErr.UNKNOWN_ERROR, e
								.getMessage());
				updateUnjoined(joinEvent, this.thisConnId, this.otherConnId, this.otherAudJoiStr.container, true);
				return;
			}

			try {
				mgcpWrapper.addListener(this.otherTx, this);
		...
		public void transactionTxTimedOut(JainMgcpCommandEvent jainMgcpComdEve) {
			...
			if (noOfRespReceived == 2) {
				JoinEvent joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
						this.otherAudJoiStr.container, container, JoinEvent.UNJOINED, false, MediaErr.TIMEOUT,
						this.errorTxt);
				updateUnjoined(joinEvent, this.thisConnId, this.otherConnId, this.otherAudJoiStr.container, true);
			}
			...
			case ReturnCode.TRANSACTION_EXECUTED_NORMALLY:
				...
				if (noOfRespReceived == 2) {
					if (!this.error) {
						...
						joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
								this.otherAudJoiStr.container, container, JoinEvent.UNJOINED, true);
						updateUnjoined(joinEvent, this.thisConnId, this.otherConnId, this.otherAudJoiStr.container,
								false);
					} else {
						joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
	...
	private class ModifyTx implements Runnable, JainMgcpExtendedListener {
		...
		public void transactionTxTimedOut(JainMgcpCommandEvent arg0) {
			...
			JoinEvent joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
					this.otherAudJoiStr.container, container, JoinEvent.JOINED, false, MediaErr.TIMEOUT,
					"No response from MGW. Tx timed out " + arg0.toString());
			updateJoined(joinEvent, this.thisConnId, this.otherConnId, this.otherAudJoiStr.container, true);
		}
		...
		private void processMofiyConnectionResponse(ModifyConnectionResponse responseEvent) {
			...
			case ReturnCode.TRANSACTION_EXECUTED_NORMALLY:
				...
				joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
						this.otherAudJoiStr.container, container, JoinEvent.JOINED, true);
				updateJoined(joinEvent, this.thisConnId, this.otherConnId, this.otherAudJoiStr.container, false);
	...
	private class JoinTx implements Runnable, JainMgcpExtendedListener {
		...
		public void transactionTxTimedOut(JainMgcpCommandEvent jainMgcpCommandEvent) {
			...
			updateJoined(joinEvent, null, null, this.otherAudJoiStr.container, true);
		}
		...
		private void processCreateConnectionResponse(CreateConnectionResponse responseEvent) {
			...
			case ReturnCode.TRANSACTION_EXECUTED_NORMALLY:
				...
				joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
						this.otherAudJoiStr.container, container, JoinEvent.JOINED, true);
				updateJoined(joinEvent, thisConnId, otherConnId, this.otherAudJoiStr.container, false);
			...
			case ReturnCode.ENDPOINT_INSUFFICIENT_RESOURCES:
				...
				joinEvent = new JoinEventImpl((ResourceContainer) container, this.context,
						this.otherAudJoiStr.container, container, JoinEvent.JOINED, false,
						MediaErr.RESOURCE_UNAVAILABLE, responseEvent.getReturnCode().getComment());
				updateJoined(joinEvent, null, null, this.otherAudJoiStr.container, true);
				break;
			default:
				mgcpWrapper.removeListener(responseEvent.getTransactionHandle());
