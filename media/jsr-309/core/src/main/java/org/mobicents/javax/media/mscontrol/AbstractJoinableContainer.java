...
public abstract class AbstractJoinableContainer implements JoinableContainer {
	...
	public void join(Direction direction, Joinable other) throws MsControlException {
		AudioJoinableStream otherStream = prepareJoin(other);
		this.audioJoinableStream.join(direction, otherStream);
	}

	private AudioJoinableStream prepareJoin(Joinable other) throws MsControlException {
		checkState();
		...
		return absJoiConOther.audioJoinableStream;
	}

	public void joinInitiate(Direction direction, Joinable other, Serializable context) throws MsControlException {
		AudioJoinableStream otherStream = prepareJoin(other);
		this.audioJoinableStream.joinInitiate(direction, otherStream, context);
	}

	public void unjoin(Joinable other) throws MsControlException {
		AudioJoinableStream otherAudioJoinableStream = prepareUnJoin(other);
		if (otherAudioJoinableStream != null) {
			this.audioJoinableStream.unjoin(otherAudioJoinableStream);
		}
	}

	private AudioJoinableStream prepareUnJoin(Joinable other) throws MsControlException {

		AudioJoinableStream otherAudioJoinableStream = null;
		if (!this.state.equals(MediaObjectState.RELEASED)) {
			AbstractJoinableContainer absJoiConOther = (AbstractJoinableContainer) other;
			otherAudioJoinableStream = absJoiConOther.audioJoinableStream;
		...
		return otherAudioJoinableStream;
	}

	public void unjoinInitiate(Joinable other, Serializable context) throws MsControlException {
		AudioJoinableStream otherAudioJoinableStream = prepareUnJoin(other);
		if (otherAudioJoinableStream != null) {
			this.audioJoinableStream.unjoinInitiate(otherAudioJoinableStream, context);
		}
	}
