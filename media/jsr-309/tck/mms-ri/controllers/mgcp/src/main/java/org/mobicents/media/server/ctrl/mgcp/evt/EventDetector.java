...
public abstract class EventDetector implements NotificationListener {
	...
	public EventName getEventName() {
		if(this.connectionIdentifier!=null){
			return new EventName(PackageName.factory(pkgName), MgcpEvent.factory(eventName), connectionIdentifier);
		}
	...
	protected boolean doVerify(Connection connection) {
            //logger.info("Verifying " + resourceName + " on connection " + connection);
		component = connection.getComponent(resourceName, Connection.CHANNEL_RX);
                if (component == null) {
                    logger.warn("Unknown component " + resourceName);
                }
		return component != null;
	}

	protected boolean doVerify(Endpoint endpoint) {
            logger.info("Verifying " + resourceName + " on endpoint " + endpoint);
