...
public abstract class RTPTrack {
	...
	public RTPTrack(TrackBox audioTrackBox, TrackBox audioHintTrackBox, File file) throws FileNotFoundException {
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Total sample count for Hint Track that should match with SampleSizeBox sampleCount("
					+ hintSampleCount + ") = " + tempCnt);
			...
			logger.debug("Total sample count for Track that should match with SampleSizeBox sampleCount("
					+ sampleTableBox.getSampleSizeBox().getSampleCount() + ") = " + tempCnt);
		}// Debug ends
		...
		for (int i = 0; i < hintChunkOffset.length; i++) {
			...
			for (int j = 1; j < samplesInThisChunk; j++) {
				if (hintSampleSize == 0) {
					hintSamplesOffSet[samplesOffSetCount++] = hintSamplesOffSet[samplesOffSetCount - 2]
							+ hintEntrySize[samplesOffSetCount - 2];
				} else {
		...
		for (int i = 0; i < audioChunkOffset.length; i++) {
			...
			for (int j = 1; j < samplesInThisChunk; j++) {
				if (audioSampleSize == 0) {
					samplesOffSet[samplesOffSetCount++] = samplesOffSet[samplesOffSetCount - 2]
							+ audioEntrySize[samplesOffSetCount - 2];
				} else {
