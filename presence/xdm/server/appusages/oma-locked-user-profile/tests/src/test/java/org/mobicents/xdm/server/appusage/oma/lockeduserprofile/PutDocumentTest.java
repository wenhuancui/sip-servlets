...
public class PutDocumentTest extends AbstractT {
	...
	private String documentName = "lockedprofile";
	...
	public void test() throws IOException, JAXBException, InterruptedException, TransformerException, NotWellFormedConflictException, NotUTF8ConflictException, InternalServerErrorException, InstanceNotFoundException, MBeanException, ReflectionException, URISyntaxException, MalformedObjectNameException, NullPointerException, NamingException {
		...
		String documentSelector = DocumentSelectorBuilder.getUserDocumentSelectorBuilder(OMALockedUserProfileAppUsage.ID,user,documentName).toPercentEncodedString();
		...
		// send put request and get response (as service provider)
		XcapResponse putResponse = client.put(documentURI,OMALockedUserProfileAppUsage.MIMETYPE,content,null,null);
		...
		// try put as the user, should receive 403
		putResponse = client.put(documentURI,OMALockedUserProfileAppUsage.MIMETYPE,content,getAssertedUserIdHeaders(client.getHeaderFactory(),user),null);
		System.out.println("Response got:\n"+putResponse);
		assertTrue("Put response must exists",putResponse != null);
		assertTrue("Put response code should be 403",putResponse.getCode() == 403);
		
		// ensure user can get doc
		XcapResponse getResponse = client.get(documentURI,getAssertedUserIdHeaders(client.getHeaderFactory(), user),null);
		assertTrue("Get response must exists",getResponse != null);
		assertTrue("Get response code should be 200",getResponse.getCode() == 200); 
		System.out.println("Response got:\n"+getResponse);
				
		// ensure user can't delete
		XcapResponse deleteResponse = client.delete(documentURI,getAssertedUserIdHeaders(client.getHeaderFactory(), user),null);
		...
		// delete doc (as service provider)
		deleteResponse = client.delete(documentURI,null,null);
