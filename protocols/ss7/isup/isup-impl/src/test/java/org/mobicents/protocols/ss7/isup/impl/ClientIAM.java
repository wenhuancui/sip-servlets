...
public class ClientIAM implements ISUPListener {
	...
	private boolean _RCV_ACM, _RCV_ANM, _RCV_TX_TERM;
	private boolean passed = true;
	private StringBuilder status = new StringBuilder();
	private ISUPClientTransaction ctx;
	
	
	public ClientIAM(ISUPStack isupStack) {
		super();
	...
	public void start() throws IllegalArgumentException, TransactionAlredyExistsException, ParameterRangeInvalidException, IOException {
		...
		nais.setContinuityCheckIndicator(NatureOfConnectionIndicators._CCI_PERFORMED_ON_PREVIOUS_CIRCUIT);
		...
		TransmissionMediumRequirement tmr = this.parameterFactory.createTransmissionMediumRequirement();
	...
	public void onMessage(ISUPMessage message) {
		...
		case AddressCompleteMessage.MESSAGE_CODE:
			...
			{
				passed = false;
				status.append("Received ACM message more than once!\n");
			}
			if(_RCV_ANM)
			{
				passed = false;
				status.append("Received ANM message before ACM!\n");
			}
			if(message.getTransaction() == null || (message.getTransaction() instanceof ISUPServerTransaction) || !message.getTransaction().equals(ctx) )
			{
				passed = false;
				status.append("Wrong transaction object on ACM "+message.getTransaction()+", local: "+this.ctx+"!\n");
			}
			_RCV_ACM = true;
			break;
		case AnswerMessage.MESSAGE_CODE:
			...
			{
				passed = false;
				status.append("Received ANM message more than once!\n");
			}
			
			if(message.getTransaction() == null || (message.getTransaction() instanceof ISUPServerTransaction) || !message.getTransaction().equals(ctx) )
			{
				passed = false;
				status.append("Wrong transaction object on ANM "+message.getTransaction()+", local: "+this.ctx+"!\n");
			}
	...
	public boolean isPassed() {
		return passed && _RCV_ACM && _RCV_ANM && _RCV_TX_TERM;
	}
	...
	public String getStatus() {
		status.append("ACM["+_RCV_ACM+"]").append("\n");
		status.append("ANM["+_RCV_ANM+"]").append("\n");
